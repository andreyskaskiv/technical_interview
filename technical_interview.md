...
AND проверяет, является ли первый операнд true либо false. И если первый операнд является true, то
AND возвращает второй операнд, потому что этого достаточно для всего условия. Если же первый операнд равняется false,
то он возвращает просто его. То есть, еще раз, первый операнд, если эквалентен false, возвращает первый операнд. Если
эквалентен true, возвращает второй операнд.
...
Как работает OR? Чтобы OR было правдой, нам достаточно, чтобы какой-то из операндов был правдой.
Соответственно, мы проверяем первый операнд. Если он эквалентен true, то мы возвращаем его. Если же он эквалентен
false, то возвращаем второй операнд.
...
# {'__module__': '__main__', 'public': 123, '_protected': 123, '_A__private': 123, 'some': <function A.some at 0x000001BA3914F880>, 'some_classmethod': <classmethod(<function A.some_classmethod at 0x000001BA3914FBA0>)>, 'some_staticmethod': <staticmethod(<function A.some_staticmethod at 0x000001BA393DD620>)>, '__dict__': <attribute '__dict__' of 'A' objects>, '__weakref__': <attribute '__weakref__' of 'A' objects>, '__doc__': None}
```
...
Если же у меня `__str__` нет, то будет метод print искать `__repr__`, который определен в данном классе.
...
общего класса. У нас есть функция some может работать со всеми объектами типов данных, у которых просто есть функция hi.
...
list_in = [1, "test value", 3, 4, 5]
print(to_json(list_in))

int_in = 5
print(to_json(int_in))

str_in = "Hi, Mr. Anderson"
print(to_json(str_in))

# [<class 'int'>,test value,<class 'int'>,<class 'int'>,<class 'int'>]
# <class 'int'>
# Hi, Mr. Anderson
```
...
Замыкание это механизм, когда одна функция использует переменная из внешней области видимости.
Например, у нас есть функция add_number, которая будет принимать какой-то aргумент а, и у нас также есть функция add,
которая будет принимать какой-то aргумент b, но в свою очередь в функции add мы используем переменную из внешней области
...
соответственно мы передаем ей a. Если мы принтанём i, то у нас есть list_iterator.
Как можно убедиться, что это на самом деле итератор? У него должен быть метод next, мы должны получить true. 
...
**48. Что такое генератор, как написать на Python генератор и какие преимущества использования генератора относительно 
функций?**  
