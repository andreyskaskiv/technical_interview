**1. Что такое pep8?**  
Pep или Python Enhancement Proposal – это предложения по развитию языка Python. pep8 – это стайл-гайд того,
как должен быть оформлен код, написан на языке Python и которому должны следовать все разработчики,
которые пишут на этом языке.
...
которую установим в глобальном у нас на компьютере.
...
Как примеры использования Листа Dict Comprehension это просто распаковка каких-то
коллекций, либо каких-то генераторов, либо просто замена какого-нибудь очень простого цикла, когда это не мешает
читабельности кода.

**6. Какая разница между операторами равно равно и из?**  
`==` - сравнивает два операнда по значению
`is` - по адрессам в памяти
...
**7. Что такое глубокая поверхностная копия, зачем они нужны и как их сделать?**
...
Как можно создать глубокую поверхностную копию?
...
**10. Как работают операторы global иnonlocal?**
...
`map` — это такая функция, которая может быть примена к коллекции. Она применяет некоторую функцию к каждому элементу
нашей коллекции.
...
Есть два типа передачи аргументов в функцию. Это по ссылке и по значению. Конкретно в Python все аргументы в
...
`*args` и `**kwargs` это особые аргументы, которые позволяют принять нам неограниченное количество
аргументов.
...
3  # получаем самый большой ключь в словаре
```
...
1  # ключь, у которого самое большое значение
```
...
использует больше памяти, нежели, например, если мы будем решать помощью использования цикла. Правила большинства
...
Метод класса это значит, что этот метод он принадлежит всему классу целиком. Он первым аргументом получает `cls`, это
ссылка на сам класс. И внутри этого метода мы имеем доступ к самому классу и к его матери класса.

**24. Чтобы реализовать статический метод?**
Статические методы по умолчанию они не принимают ничего. Да, они конечно могут принимать какие-то аргументы, которые им
